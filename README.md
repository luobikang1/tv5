...
> ```text
> ┌────────────────────────────────────────────────────────────────────────┐
> │ 数据库同步失效，暂未解决。系统已启用本地 LocalStorage 离线备份模式。 │
> └────────────────────────────────────────────────────────────────────────┘
> ```
>
> **2. 压缩分辨率与切片调节功能**：
...
> ┌────────────────────────────────────────────────────────────────────────┐
> │ 压缩分辨率功能无效，仅预留操作按钮。                                  │
> │ 但接入 Cloudflare R2 对象存储时可用（实现转码切片与对象存储代理）。  │
> └────────────────────────────────────────────────────────────────────────┘
> ```
>
> **3. Cloudflare R2 出口费用预警标识**：
> ```text
> ┌────────────────────────────────────────────────────────────────────────┐
> │ 接入 CF R2 对象存储时，面板将在当月出口数据传输量超过 10GB 时显示     │
> │ 醒目的【超过 10GB 免费流量出口预警 ⚠️】标识，防止意外产生超出费。      │
> └────────────────────────────────────────────────────────────────────────┘
> ```
...
5. **设置 API / D1 同步与 R2 对象存储**：
   - 在 Cloudflare Pages 项目设置中 -> **Functions** -> **D1 database bindings** -> 绑定名为 `DB` 的 D1 数据库（当前数据库同步失效暂未解决，可保留绑定或使用离线存储）。
