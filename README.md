...
> ```
>
> **3. Cloudflare R2 出口费用预警标识与开关**：
> ```text
> ┌────────────────────────────────────────────────────────────────────────┐
> │ 设置面板中支持 Cloudflare R2 存储独立开关。接入 CF R2 对象存储时，   │
> │ 当月出口数据传输量超过 10GB 时显示【超过 10GB 免费流量出口预警 ⚠️】。   │
> └────────────────────────────────────────────────────────────────────────┘
> ```
...
1. **Cloudflare Pages 代码部署**：
   ...
   - **绑定 D1 数据库**：在 Pages 设置中绑定名称为 `DB` 的 D1 数据库后，**请务必点击重新部署 (Redeploy) 项目**以确保 D1 完全生效。
